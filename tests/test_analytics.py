...
class TestAnalytics:
    ...
    async def test_user_analytics(self, client: AsyncClient, auth_headers: dict):
        """
        Check /api/analytics/user.
        If the endpoint is not yet implemented — allow 404 to not break the entire run.
        When you implement it — just change the check to strict 200.
        """
    ...
    async def test_post_analytics(self, client: AsyncClient, auth_headers: dict):
        # Test allows 200 (if post exists) or 404 (if none exists)
    ...
    async def test_comments_daily_breakdown(self, client: AsyncClient):
        # According to STEP 5: /api/comments-daily-breakdown
